...
namespace multiverso { namespace lightlda
{
    ...
    {
    public:
        MemoryDataStream(int32_t num_blocks, lightlda::dump* dump);
    ...
    MemoryDataStream::MemoryDataStream(int32_t num_blocks, lightlda::dump *dmp)
        : data_path_(std::string("")), index_(0)
    {
        data_buffer_.resize(num_blocks, nullptr);
        for (int32_t i = 0; i < num_blocks; ++i)
        {
...
//            data->Write();
    ...
    IDataStream* CreateDataStream(dump *dmp)
    {
        if (Config::out_of_core && Config::num_blocks != 1)
        {
            return new DiskDataStream(Config::num_blocks, Config::input_dir,
        ...
        {
            return new MemoryDataStream(Config::num_blocks,dmp);
        }
    }
} // namespace lightlda
